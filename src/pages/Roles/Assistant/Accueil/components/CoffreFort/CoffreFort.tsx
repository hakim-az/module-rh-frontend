...
export default function CoffreFortTable({
  ...
  const table = useReactTable({
    data: dashboardData?.latest.coffres ?? [],
    columns,
  ...
  return (
    <div>
      <div className="w-full flex items-center justify-between mb-6">
        <span className="text-xl font-semibold">Coffres fort</span>
        <button
          type="button"
          onClick={() => navigate('coffre-fort')}
          className="flex items-center justify-center gap-3 bg-[#2A9D8F] text-white px-5 py-2 rounded hover:scale-110 transition-all ease-in-out delay-75">
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-[300px]">
                  <div className="flex items-center justify-center w-full h-full text-red-500 text-xl">
                    Une erreur est survenue lors du chargement des coffres
                    forts.
                  </div>
                </TableCell>
              </TableRow>
              ...
              <NotFoundTable
                ...
                title="Coffres fort introuvables"
                content="Aucun coffre fort n'a été trouvé."
              />
            )}
          </TableBody>
        </Table>
